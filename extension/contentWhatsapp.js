console.log("contentWhatsapp.js loaded on WhatsApp");
...
function monitorMessages() {
    ...
    const observer = new MutationObserver(() => {
        ...
        messageContainers.forEach((container) => {
            ...
            if (!messageList.some(msg => msg.sender === sender && msg.text === text)) {
                messageList.push({ sender, text });
                ...
                fetch("http://127.0.0.1:8000/api/messages/", {
                    ...
                    body: JSON.stringify({ "conversation": messageList })
                })
                    .then(response => {
                        if (!response.ok) {
